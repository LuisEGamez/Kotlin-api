...
class GetFindCourseByIdControllerTest {
    ...
    private fun `then a not found response is returned`(actualResponse: ResponseEntity<CourseResponse>) {
        assertEquals(
            ResponseEntity.status(HttpStatus.NOT_FOUND).build(), actualResponse
        )
    }
    ...
    companion object {
        ...
        private val course = CourseResponse(
            id = courseId, name = "Kotlin - Your first API", createdAt = LocalDateTime.parse("2022-08-31T09:07:36"), description = "Description"
        )
    }
}
